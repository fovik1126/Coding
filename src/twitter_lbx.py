...
class CNN_Fusion(nn.Module):
    def __init__(self, args):
        ...
        for name, para in self.resnet_cbam50.named_parameters():
    ...
    def forward(self, text, image, ocr, mask):
        ...
        image_norm = torch.sqrt(torch.sum(torch.pow(image, 2), dim=1))
        ...
        distance = torch.stack([distance_, cos_simi], dim=1)
        ...
        ocr_image = torch.sum(torch.mul(ocr, image), dim=1)
        cos_simi_ocr_img = (1 + (ocr_image / (ocr_norm * img_norm + 1e-8))) / 2
        distance_ocr_img = torch.ones_like(cos_simi_ocr_img) - cos_simi_ocr_img
        distance_ocr_img_ = torch.stack([distance_ocr_img, cos_simi_ocr_img], dim=1)

        return class_output, distance, distance_ocr_, distance_ocr_img_
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            class_outputs, distance, distance_ocr_, distance_ocr_img_ = model(train_text, train_image, train_ocr,
                                                                              train_mask)
            ...
            ocr_img_cos_sim_loss = criterion(distance_ocr_img_, train_labels.long())

            loss = 0.6 * class_loss + 0.2 * cos_sim_loss + 0.1 * ocr_text_cos_sim_loss + 0.1 * ocr_img_cos_sim_loss
            loss.backward()
            optimizer.step()
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            ...
            validate_outputs, distance, distance_ocr_, distance_ocr_img_ = model(validate_text, validate_image,
        ...
        model.train()
        print(
            'Epoch [%d/%d],  Loss: %.4f, Class Loss: %.4f, cos_sim loss: %.4f, ocr_text loss: %.4f , Train_Acc: %.4f,  Validate_Acc: %.4f.'
            % (
                epoch + 1, args.num_epochs, np.mean(cost_vector), np.mean(class_cost_vector),
                np.mean(cos_sim_cost_vector),
                np.mean(ocr_text_cos_sim_cost_vector),
    ...
    for i, (test_data, test_labels, event_labels) in enumerate(test_loader):
        ...
        test_outputs, distance, distance_ocr_, distance_ocr_img_ = model(test_text, test_image, test_ocr, test_mask)
