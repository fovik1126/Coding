...
class CNN_Fusion(nn.Module):
    def __init__(self, args):
        ...
        self.class_Multimodal_classifier.add_module('c_fc1', nn.Linear(4 * self.hidden_size, 2))
        self.class_Multimodal_classifier.add_module('c_softmax', nn.Softmax(dim=1))

        self.class_classifier = nn.Sequential()
        self.class_classifier.add_module('c_fc2', nn.Linear(3 * self.hidden_size, 2))
    ...
    def forward(self, text, image, ocr, mask):
        ...
        Score = self.GatedFusionGlobal(cross_att_image_text, cross_att_text_image, ocr, True, "words")
        # print(Score.shape) [64]
        scaled_score = torch.sigmoid(torch.mean(Score))
        # print(scaled_score) tensor(0.6157, device='cuda:0', grad_fn=<SigmoidBackward0>)
        # print(score.shape) #[128]

        Multimodal = self.GatedFusionGlobal(cross_att_image_text, cross_att_text_image, ocr, False, "words")
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            loss = scaled_score * class_loss + domain_loss  # 0.918
