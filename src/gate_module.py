...
class GatedFusionGlobal(nn.Module):
    def __init__(self, dim, num_attn, dropout=0.01, reduce_func="mean", fusion_func="concat"):
        ...
        self.extra_linear_layer = nn.Linear(32, 2 * dim)
        # if reduce_func == "mean":
        #     self.reduce_layer = torch.mean
        # elif reduce_func == "self_attn":
        #     self.reduce_layer_1 = SummaryAttn(dim, num_attn, dropout)
        ...
        if reduce_func == "mean":
            ...
            raise NotImplementedError('Only support mean or self_attn reduction')

        self.init_weights()
        print("GatedFusionGlobal module init success!")

    def init_weights(self):
        r = np.sqrt(6.) / np.sqrt(self.dim + self.dim)
        self.img_key_fc.weight.data.uniform_(-r, r)
        self.txt_key_fc.weight.data.uniform_(-r, r)
        self.fc_1[0].weight.data.uniform_(-r, r)
        self.fc_1[0].bias.data.fill_(0)
    ...
    def forward(self, global_img_features, global_txt_features_1, global_txt_features_2, get_score=True, keep=None):
        # 获取各输入的批次大小
        batch_size_v1 = global_img_features.size(0)  # 【32】
        batch_size_v2_1 = global_txt_features_1.size(0)  # 【32】
        batch_size_v2_2 = global_txt_features_2.size(0)  # 【32】

        # 在维度1上扩展图像特征和关键信息
        v1 = global_img_features.unsqueeze(1).expand(batch_size_v1, batch_size_v2_1 + batch_size_v2_2, -1)  # [32,64,32]

        # 在维度0上扩展文本关键信息
        k2_1 = global_txt_features_1.unsqueeze(0).expand(batch_size_v1, batch_size_v2_1, -1)  # [32,32,32]
        k2_2 = global_txt_features_2.unsqueeze(0).expand(batch_size_v1, batch_size_v2_2, -1)  # [32,32,32]

        # 门控计算
        gate_v1_1 = F.sigmoid((v1[:, :batch_size_v2_1, :] * k2_1).sum(dim=-1)).unsqueeze(-1)  # [32,32,1]
        gate_v1_2 = F.sigmoid((v1[:, batch_size_v2_1:, :] * k2_2).sum(dim=-1)).unsqueeze(-1)  # [32,32,1]

        # 融合图像和文本特征
        fused_v1_1 = torch.cat((v1[:, :batch_size_v2_1, :], k2_1), dim=-1) * gate_v1_1  # [32,32,64]
        fused_v1_2 = torch.cat((v1[:, batch_size_v2_1:, :], k2_2), dim=-1) * gate_v1_2  # [32,32,64]

        # 使用全连接层进行进一步融合
        co_v1_1 = self.fc_1(fused_v1_1) + v1[:, :batch_size_v2_1, :]  # [32,32,32]
        co_v1_2 = self.fc_1(fused_v1_2) + v1[:, batch_size_v2_1:, :]  # [32,32,32]

        if self.reduce_func == "mean":
            co_v1_1 = self.reduce_layer(co_v1_1, dim=-2)  # [32,32]
            co_v1_2 = self.reduce_layer(co_v1_2, dim=-2)  # [32,32]
        elif self.reduce_func == "self_attn":
            co_v1_1 = self.reduce_layer_1(co_v1_1)
            co_v1_2 = self.reduce_layer_2(co_v1_2)

        # 归一化
        co_v1_1 = l2norm(co_v1_1)
        co_v1_2 = l2norm(co_v1_2)  # [32,32]

        # 额外的线性层
        co_v1_1 = self.extra_linear_layer(co_v1_1)  # [32,64]
        co_v1_2 = self.extra_linear_layer(co_v1_2)  # [32,64]

        if get_score:

            # 计算得分
            score_1 = self.fc_out(co_v1_1).squeeze(dim=-1)  # [32]
            score_2 = self.fc_out(co_v1_2).squeeze(dim=-1)  # [32]

            if self.fusion_func == "sum":
                score = score_1 + score_2  # [64]
            elif self.fusion_func == "concat":
            ...
            return torch.cat((co_v1_1, co_v1_2), dim=-1)  # [32,128]


class GatedFusionGlobal2(nn.Module):
    def __init__(self, dim, num_attn, dropout=0.01, reduce_func="mean", fusion_func="concat"):
        super(GatedFusionGlobal2, self).__init__()
        self.dim = dim
        self.h = num_attn
    ...
    def forward(self, global_img_features, global_txt_features_1, global_txt_features_2, get_score=True, keep=None):
        ...
        # 在维度1上扩展图像特征和关键信息
        v1 = global_img_features.unsqueeze(1).expand(batch_size_v1, batch_size_v2_1, -1)  # [32,32,64]

        # 在维度0上扩展文本关键信息
        k2_1 = global_txt_features_1.unsqueeze(0).expand(batch_size_v1, batch_size_v2_1, -1)  # [32,32,32]

        # 门控计算
        gate_v1_1 = F.sigmoid((v1 * k2_1).sum(dim=-1)).unsqueeze(-1)  # [32,32,1]

        # 融合图像和文本特征
        fused_v1_1 = torch.cat((v1, k2_1), dim=-1) * gate_v1_1  # [32,32,96]

        # 使用全连接层进行进一步融合
        co_v1_1 = self.fc_1(fused_v1_1) + v1  # [32,32,64]
        ...
        if get_score:
            ...
            # 计算得分
            score = self.fc_out(co_v1_1).squeeze(dim=-1)  # [32]

            if keep == "regions":
                score = score.transpose(0, 1)
            return score
        else:
            return co_v1_1  # [32,64]
