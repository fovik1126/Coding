...
class CNN_Fusion(nn.Module):
    ...
    def forward(self, text, image, ocr, mask):
        ...
        # print(Multimodal.shape)#[32, 128]
        # Multimodal = score * Multimodal
        # print(Multimodal.shape)#[32, 128]
        # Multimodal = torch.cat((cross_att_image_text, cross_att_text_image, ocr), 1)
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            loss = scaled_score * class_loss + domain_loss
            # loss = 0 * cos_sim_loss + 0.6 * class_loss + 0.2 * ocr_text_cos_sim_loss + 0.2 * ocr_img_cos_sim_loss
            loss.backward()
            optimizer.step()
        ...
        # ---------------------------------------画图-------------------------
        writer = SummaryWriter(log_dir="../Data/twitter_visual")
