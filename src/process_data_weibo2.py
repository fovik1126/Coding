...
def stopwordslist(filepath='../Data/weibo_image/stop_words.txt'):
    stopwords = {}
...
def read_image():
    image_list = {}
    file_list = ['../Data/weibo_image/nonrumor_images/', '../Data/weibo_image/rumor_images/']
...
def read_ocr():
    ...
    train = 'D:/papercode/BDANN-IJCNN2020-main/Data/weibo/weibo_ocr.csv'
    df = pd.read_csv(train)
    for i in range(len(df["values"][:])):
        a = clean_str_sst(df["values"][i])
        a = jieba.cut_for_search(a)
        new_seg_list = []
        df["id"][i]=df["id"][i].lower()
        for word in a:
            if word not in stop_words:
                new_seg_list.append(word)
        seged_line = " ".join(new_seg_list)
        image_dict_[df["id"][i]] = seged_line
    return image_dict_

def write_txt(data):
    f = open("../Data/weibo_image/top_n_data.txt", 'wb')
    for line in data:
        for l in line:
            f.write(l + "\n")
        f.write("\n")
...
def write_data(flag, image, image_dict_, text_only):
    def read_post(flag):
        stop_words = stopwordslist()
        ...
        deleted_data = pickle.load(open("../Data/weibo/deleted_post_fold_1.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(open("../Data/weibo/deleted_post_fold_2.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(open("../Data/weibo/deleted_post_fold_3.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(open("../Data/weibo/deleted_post_fold_4.pkl", 'rb'))
        deleted_data = deleted_data + pickle.load(open("../Data/weibo/deleted_post_fold_5.pkl", 'rb'))
    ...
    def paired(text_only=False):
        ...
        for i, id in enumerate(post['post_id']):
            ...
            if text_only or image_id in image and image_dict_:
                if not text_only:
                    image_name = image_id
                    image_id_list.append(image_name)
                    ordered_image.append(image[image_name])
                    ocr.append(image_dict_[image_name])
...
def build_data_cv(data_folder, cv=10, clean_string=True):
    ...
    vocab = defaultdict(float)
    with open(pos_file, "rb",encoding='utf-8') as f:
        ''
        for line in f:
            rev = []
            ...
            revs.append(datum)
    with open(neg_file, "rb",encoding='utf-8') as f:
        for line in f:
            rev = []
            rev.append(line.strip())
            if clean_string:
...
def get_data(text_only):
    ...
    if text_only:
        ...
        image_dict_ = read_ocr()
    train_data = write_data("train", image_list, image_dict_, text_only)
    valiate_data = write_data("validate", image_list, image_dict_, text_only)
    test_data = write_data("test", image_list, image_dict_, text_only)
