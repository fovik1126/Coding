...
#    stop_words = stopwordslist()
    train = 'D:/papercode/BDANN-IJCNN2020-main/twitter_ocr.csv'
...
def write_data(flag, image, image_dict_, text_only):
    ...
    def read_post(flag):
        pre_path = "../Data/twitter/"
        if flag == "train":
            texts = open(pre_path + 'train_posts.txt', 'rb').readlines()
            ...
            texts = open(pre_path + 'test_posts.txt', 'rb').readlines()
        ...
        for i, line in enumerate(texts):
            ...
            if flag == 'train' or flag == 'validate':
                image_id = line.decode().split('\t')[3].lower().split(',')
            else:
                image_id = line.decode().split('\t')[4].lower().split(',')
            continue_flag = False
            ...
            post_id = line.decode().split('\t')[0]
            text = text_dict[post_id]

            if len(text.split(' ')) <= 2:
                # print(text)
                continue
            label = 0 if line.decode().split('\t')[-1].strip() == 'real' else 1
...
def get_data(text_only):
    ...
    train_data, event_num = write_data("train", image_list, image_dict_, text_only)
    # valiate_data, _ = write_data("validate", image_list, text_only)
    # test_data, _ = write_data("test", image_list, text_only)
    valiate_data, _ = write_data("validate", image_list, image_dict_, text_only)
    test_data, _ = write_data("validate", image_list, image_dict_, text_only)
