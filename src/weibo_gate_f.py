...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            loss = 0.6 * class_loss + 0.4 * domain_loss  # 0.908
            #loss = class_loss + domain_loss #0.902
            #loss = 0.4*class_loss + 0.6 * domain_loss #0.907
            #loss = 0 * class_loss + domain_loss
            # loss=class_loss+0*domain_loss
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            ...
            validate_outputs, distance_ = model(validate_text, validate_image, validate_ocr, validate_mask)
