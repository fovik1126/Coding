...
class CNN_Fusion(nn.Module):
    def __init__(self, args):
        ...
        self.class_Multimodal_classifier.add_module('c_fc1', nn.Linear(8 * self.hidden_size, 2))
    ...
    def forward(self, text, image, ocr, mask):
        ...
        # gate文本
        Multimodal_text = self.GatedFusionGlobal2(text, cross_att_text_image, False, "words")
        # gate图像
        Multimodal_image = self.GatedFusionGlobal2(cross_att_image_text, image, False, "words")

        Fm1 = torch.cat((Multimodal_text, Multimodal_image), 1)  # (32,128)

        # gate文本OCR
        Multimodal_ocr = self.GatedFusionGlobal2(ocr, cross_att_text_image, False, "words")
        # gate图像
        Multimodal_image_ = self.GatedFusionGlobal2(image, ocr, False, "words")

        Fm2 = torch.cat((Multimodal_ocr, Multimodal_image_), 1)  # (32,128)

        Multimodal = torch.cat((Fm1, Fm2), dim=-1)  # (32,256)

        score_1 = self.fc_out(Fm1).squeeze(dim=-1)  # (32,1)
        score_2 = self.fc_out(Fm2).squeeze(dim=-1)  # (32,1)

        score = torch.cat((score_1, score_2), dim=-1)  # (32,2)

        scaled_score = torch.sigmoid(torch.mean(score))  # 0-1之间的标量

        Unimodal = torch.cat((text, image, ocr), 1)
        # print(Unimodal.shape)  # torch.Size([32, 96])
        ...
        return Multimodal_output, Unimodal_output, scaled_score


# result = Score * Multimodal + Unimodal
def to_var(x):
...
def main(args):
    ...
    print("bach总数：", "训练集" + str(len(train_loader)), "验证集" + str(len(validate_loader)),
          "测试集" + str(len(test_loader)))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            # #class_outputs, distance, distance_ocr_, distance_ocr_img = model(train_text, train_image, train_ocr,
            #                                                                  train_mask)
            Multimodal_output, Unitimodal_output, scaled_score = model(train_text, train_image, train_ocr,
                                                                       train_mask)
            # Fake or Real loss
            ...
            domain_loss = criterion(Unitimodal_output, train_labels.long())

            loss = scaled_score * class_loss + domain_loss  # 0.918   0.929
            # loss = class_loss + domain_loss #0.895
            # loss = class_loss + 0 * domain_loss #0.871
            # loss = 0 * class_loss + domain_loss #0.587

            loss.backward()
            optimizer.step()
            _, argmax = torch.max(Multimodal_output, 1)
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            ...
            validate_outputs, domain_outputs, scaled_score = model(validate_text, validate_image,
                                                                   validate_ocr, validate_mask)
        ...
        model.train()
        print(
            'Epoch [%d/%d],  Loss: %.4f, Class Loss: %.4f, Unitimodal Loss : %.4f,Train_Acc: %.4f,  Validate_Acc: %.4f.'
            % (
        ...
        writer.add_scalar("untilmodal", np.mean(domain_cost_vector), epoch)
    ...
    for i, (test_data, test_labels, event_labels) in enumerate(test_loader):
        ...
        #                                                                 test_mask)
        test_outputs, domain_outputs, scaled_score = model(test_text, test_image, test_ocr,
                                                           test_mask)
